...
async def delete_bus(id: int) -> str:

    deletescript = """
        DELETE FROM [transporte].[bus]
        WHERE [ID] = ?;
    """
...
async def update_bus( bus : Bus ) -> Bus:

    dict = bus.model_dump(exclude_none=True)
    ...
    updatescript = f"""
        UPDATE [transporte].[bus]
        SET {variables}
        WHERE [ID] = ?;
    """

    params = [ dict[v] for v in keys ]
    params.append(bus.ID)
...
async def create_bus(bus: Bus) -> Bus:
    sqlscrip = """  
    INSERT INTO [transporte].[bus](
            [placa],
            [modelo],
            [capacidad]
    )VALUES(
...
async def get_one_recorrido_bus( bus_ID: int, recorrido_ID: int ) -> Recorrido:
    selectscript = """
      SELECT
            r.ID,
            b.ID as ID_Bus,
...
async def get_all_recorrido_bus( bus_ID: int) -> Recorrido:
    selectscript = """
      SELECT
            r.ID,
            b.ID as ID_Bus,
